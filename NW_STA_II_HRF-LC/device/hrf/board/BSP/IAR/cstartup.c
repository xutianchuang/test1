...
static  void  save_crash_info_and_reset(u32 *stack_frame_p, u32 exc_lr);
...
/*
*********************************************************************************************************
*                                      save_crash_info_and_reset()
*
* Description : Save crash registers to a fixed SRAM area and trigger a soft reset.
*               Called from fault handlers with the exception stack frame pointer.
*
* Argument(s) : stack_frame_p  pointer to the exception stack frame {R0,R1,R2,R3,R12,LR,PC,xPSR}
*               exc_lr         the EXC_RETURN value (LR on exception entry)
*
* Note(s)     : The SRAM area at CRASH_INFO_ADDR survives a warm reset.
*********************************************************************************************************
*/
...
static  void  save_crash_info_and_reset(u32 *stack_frame_p, u32 exc_lr)
{
    ...
    info_p->r0    = stack_frame_p[0];
    info_p->r1    = stack_frame_p[1];
    info_p->r2    = stack_frame_p[2];
    info_p->r3    = stack_frame_p[3];
    info_p->r12   = stack_frame_p[4];
    info_p->lr    = stack_frame_p[5];
    info_p->pc    = stack_frame_p[6];
    info_p->xpsr  = stack_frame_p[7];

    info_p->cfsr  = SCB->CFSR;
    info_p->hfsr  = SCB->HFSR;
    info_p->mmfar = SCB->MMFAR;
    info_p->bfar  = SCB->BFAR;
    info_p->exc_lr = exc_lr;

    /*
     * Stack backtrace: scan from above the hardware-pushed exception frame
     * upward (growing addresses = older frames). Each 32-bit value that
     * falls in the code region [0x10000000, 0x10080000) with bit[0]=1
     * (Thumb return address) is likely a BL-pushed LR, i.e. a caller.
     *
     * stack_frame points to {R0,R1,R2,R3,R12,LR,PC,xPSR} (8 words).
     * Start scanning from stack_frame+8 (above the exception frame).
     * Scan up to CRASH_BT_SCAN_WORDS words (1 KB) to cover typical call depths.
     */
    scan_p = stack_frame_p + 8;
    for (i = 0; i < CRASH_BT_SCAN_WORDS && depth < CRASH_BT_DEPTH_MAX; i++)
    {
        /* Prevent reading beyond reserved SRAM area */
        if ((u32)&scan_p[i] >= CRASH_SRAM_SCAN_LIMIT)
            break;
        val = scan_p[i];
        /* Check: in code region and Thumb bit set (BL return address) */
        if ((val & CODE_REGION_MASK) == CODE_REGION_BASE && (val & 1) == 1)
        {
            /* Skip if identical to already captured PC or LR */
            if (val != info_p->pc && val != info_p->lr)
            {
                info_p->backtrace[depth++] = val;
            }
        }
    }
    info_p->bt_depth = depth;

    info_p->magic = CRASH_INFO_MAGIC;

    /* Set FALUT_RESET_FLAG at ResetAddr */
    RESET_ADDR_PTR = FALUT_RESET_FLAG;
...
*                                             App_Fault_ISR()
*
* Description : Handle hard fault. Captures the exception stack frame and saves crash info to SRAM.
*
* Note(s)     : On Cortex-M, hardware pushes {R0,R1,R2,R3,R12,LR,PC,xPSR} onto the active stack.
*               We check LR bit[2] to determine if MSP or PSP was used.
*********************************************************************************************************
*/

#pragma required=save_crash_info_and_reset
static  void  App_Fault_ISR (void)
{
    /* IAR inline asm: get correct stack pointer, then call save_crash_info_and_reset */
    asm("TST   LR, #4        \n"  /* Test bit 2 of EXC_RETURN */
        "ITE   EQ             \n"
        "MRSEQ R0, MSP        \n"  /* If 0, exception used MSP */
        "MRSNE R0, PSP        \n"  /* If 1, exception used PSP */
        "MOV   R1, LR         \n"  /* Pass EXC_RETURN as 2nd arg */
        "B     save_crash_info_and_reset \n");
}
...
*                                          App_UsageFault_ISR()
*
* Description : Handle usage fault. Captures exception frame via save_crash_info_and_reset.
*********************************************************************************************************
*/
...
*                                           App_MemFault_ISR()
*
* Description : Handle memory fault. Captures exception frame via save_crash_info_and_reset.
*********************************************************************************************************
*/
