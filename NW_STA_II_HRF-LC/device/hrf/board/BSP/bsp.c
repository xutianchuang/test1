...
void DetectADC12VIrq(void)
{
	int current_power=0;
	OS_ERR err;
	//´Ë´¦ÓÉÓÚPINVDCFGRÓÐ±£»¤Ö±½ÓÐ´flag²»»áÇå³ýÆäËûµÄbitÎ» µ«ÊÇ±£ÏÕÆð¼ûÇå³ý×´Ì¬Î»»¹ÊÇÐèÒª°ÑÆäËûµÄbitÉèºÃ
	uint32_t reg_value=CPM->PINVDCFGR;

	if ((reg_value&(CPM_PINVDCFGR_PinVolDetHFlag_Msk|CPM_PINVDCFGR_PinVolDetLFlag_Msk))==(CPM_PINVDCFGR_PinVolDetHFlag_Msk|CPM_PINVDCFGR_PinVolDetLFlag_Msk))//Á½¸ö±í±êÖ¾Í¬Ê±ÖÃÎ»
	{
		if (power)
		{
			current_power=0;
	...
	else if (reg_value & CPM_PINVDCFGR_PinVolDetLFlag_Msk) //Í£µç
	{
		current_power=0;
		EventClose();
		ResetInterrupt(DISABLE);
