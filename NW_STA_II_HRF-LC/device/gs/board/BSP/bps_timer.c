...
{
#if ZB202
	if (Timer>BSP_TIMER_5)
	{
		return;
	}
	if (isPwmFunc)
	{
		SCU->EX_PERIPH_PIN_REMAP0 &= ~(1 << (26 + Timer));
	}
	else
	{
		SCU->EX_PERIPH_PIN_REMAP0 |= 1 << (26 + Timer);
...
#elif DM750
	if ((Timer<BSP_TIMER_6) || (Timer>BSP_TIMER_11))
	{
		return;
	}
	Timer -= BSP_TIMER_6;
	SCU->EX_PERIPH_PIN_REMAP0 &= ~(3 << (0 + Timer*4));

	if (isPwmFunc)
	{
		SCU->EX_PERIPH_PIN_REMAP0 |= (2 << (0 + Timer*4));
	}
	else
	{
		SCU->EX_PERIPH_PIN_REMAP0 |= (1 << (0 + Timer*4));
	}	
#endif
}
...
void TimerPwmOpen(TimerParameterTypdef *TimerParameter,uint32_t CmpTime20ns)
{
    ...
    CHL_InitStruct.DeadZone = 0;
    CHL_InitStruct.CfgValue = /*TIM_INT_ENABLE|*/TIM_AUTO_RELOAD_ON|TIM_UPDATA|TIM_PWM_ENABLE;
#if DM750
	CHL_InitStruct.CfgValue |= TIM_OUTPUT_INVERTER_ON;
#endif		
    CHL_InitStruct.TimReload = TimerParameter->TimerSlot20ns;
    CHL_InitStruct.CmpValue = CmpTime20ns;
...
void TimerPwmReset(TimerParameterTypdef *TimerParameter)
{
    ...
    CHL_InitStruct.DeadZone = 0;
    CHL_InitStruct.CfgValue = TIM_AUTO_RELOAD_OFF|TIM_UPDATA|TIM_PWM_ENABLE|TIM_START;
