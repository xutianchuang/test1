...
*/	
	...
	if (uart->info->xfer.dma_lli == NULL)
	{
		apDMA_MemPeripheralTransferRequest(
			uart->dma_rx->channel,
			uart->dma_rx->peripheral,
	...
	{
		((apDMA_sPerMemLLI*)(uart->info->xfer.dma_lli))[0].NumTransfers   = num;
		((apDMA_sPerMemLLI*)(uart->info->xfer.dma_lli))[0].pMemAddress    = data;
		((apDMA_sPerMemLLI*)(uart->info->xfer.dma_lli))[0].pNextLLI       = &((apDMA_sPerMemLLI*)(uart->info->xfer.dma_lli))[1];
		((apDMA_sPerMemLLI*)(uart->info->xfer.dma_lli))[1].NumTransfers   = num;
		((apDMA_sPerMemLLI*)(uart->info->xfer.dma_lli))[1].pMemAddress    = &((char*)data)[num];
		((apDMA_sPerMemLLI*)(uart->info->xfer.dma_lli))[1].pNextLLI       = &((apDMA_sPerMemLLI*)(uart->info->xfer.dma_lli))[0];
		apDMA_LinkedMemPeripheralTransferRrequest(
			uart->dma_rx->channel,
			uart->dma_rx->peripheral,
			uart->dma_rx->user_id,
			apDMA_PERIPHERAL_TO_MEM_DMA_CTRL,
			2,
			(apDMA_sPerMemLLI*)(uart->info->xfer.dma_lli),
			true);
	}
...
static int32_t USARTx_Control(USART_RESOURCES  *uart,uint32_t control, uint32_t arg)
{
	...
	{
	case ARM_USART_CONTROL_RX_LLI:
		uart->info->xfer.dma_lli = (uint32_t *)arg;
		return ARM_DRIVER_OK;
								   ...
								   uint32_t    TransferCount)
{
	...
	if (uart)
	{
		if (isRx)//接收满
		{
		  	if((apDMA_eError)ResultCode==apERR_DMA_TRANSFER_ABORTED)
			{
			  return;
			}
			uint32_t event;
			if (uart->info->rx_status.rx_busy)
			{
				if (uart->info->xfer.dma_lli)//启动了链模式
				{
					//apDMA_GetCount(uart->dma_rx->channel,&TransferCount);
					uart->info->xfer.rx_cnt = uart->info->xfer.rx_num;
					if (uart->info->xfer.lli_idx&1)
					{
						event=ARM_USART_EVENT_RECEIVE_COMPLETE;
					}
					else
					{
						event=ARM_USART_EVENT_RECEIVE_HALF_COMPLETE;
					}
				}
				else
				{
					uart->info->xfer.rx_cnt+= TransferCount;
					//close rx dma
...
static void USARTx_IRQHandler(USART_RESOURCES *uart)
{
	...
	if (status & SCI_SCISR1_IDLE_Msk)
	{
		if (uart->info->rx_status.rx_busy)
		{
			...
			if (uart->info->xfer.dma_lli)//启动了链模式
			{
				apDMA_GetCount(uart->dma_rx->channel,&count);
				uart->info->xfer.rx_cnt = uart->info->xfer.rx_num - count; //链模式dma返回的是剩余的字节数
				if (uart->info->xfer.rx_cnt==0)//没有新的数据进来 传输数据长度刚好是buffer的整数倍 在dma里已经调用过回调
				{
					event |=ARM_USART_EVENT_RECEIVE_NO_DATA_COMPLETE;
				}
				else if (uart->info->xfer.lli_idx & 1)
				{
					event |=ARM_USART_EVENT_RECEIVE_COMPLETE;
				}
				else
