...
void AppLocalHandleIFR(void)
{
	...
	if(IRF_send_data_end_flag)
	{
		...
		if (err==OS_ERR_NONE)
		{
			...
			if(Is645Frame(LocalReceiveIFRBuf,len))//是645報文
			{
				for(; index < len; index++)
				{
					if(LocalReceiveIFRBuf[index] == 0x68)break;				
				}
				if(protocol645_dispose_IFR((struct frame645 *)&LocalReceiveIFRBuf[index]))//是讀II采内部信息
				{
					m_len = 12 + index + 2 + LocalReceiveIFRBuf[10 + index + 2];                
				...
				{
					memadd(&LocalReceiveIFRBuf[10 + index],0x33,LocalReceiveIFRBuf[9 + index]);
					TransIFR2Rs485(&LocalReceiveIFRBuf[index],len-index);			
				}
			}
