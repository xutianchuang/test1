...
u32 FreTimeout = DEFULT_CHANGE_FRE_TIME; //初始使用频段1发送测试帧
#else
...
{
#ifdef II_STA
	u8 MeterNum = SEARCH_METER_FLAG.meter_total; // 取电表总数
...
#ifdef II_STA
	APP_METER_INFO meterInfo[32]; // 电表信息块
	u8 i = 0;
	u8 j = 0;
	for (i = 0; i < 32; i++)
	{
		...
		{
			if (j >= MeterNum) // 不能超过了电表总数
			{
				break;
			}
			memcpy((u8 *)(meterInfo[j].MeterMac), &SEARCH_METER_FLAG.table[i * 7], 6);
			meterInfo[j].Reserve = 0;
			// TransposeAddr(meterInfo[j].MeterMac);
			j++;
		}
