...
//map中插入一个数(map数组，maxsize,num 数组元素个数,dat 要插入的数，返回下标)
EN_INSERT_RESULT InsertMap16(u16 map[],u16 maxsize,u16 *num,u16 dat,u16 *datIndex)
{
    u16 index = 0;
    u16 min = 0;
    u16 max = 0;
    ...
    if(map[max] > dat)
    {
        memmove(&map[max + 1],&map[max],(*num - max)*sizeof(u16));
        map[max] = dat;
    }
    else if(map[max] < dat)
    {
        memmove(&map[max + 2],&map[max + 1],(*num - max - 1)*sizeof(u16));
        max++;
        map[max] = dat;
...
{
    u16 index = 0;
    if(FindMap16(map,*num,dat,&index))
    {
        memmove(&map[index],&map[index + 1],(*num - index - 1)*sizeof(u16));
        *num = *num - 1;
        return true;
    }
    return false;
...
EN_INSERT_RESULT InsertMap32(u32 *map,u16 maxsize,u16 *num,u32 dat,u16 *datIndex)
{
    u16 index = 0;
    u16 min = 0;
    u16 max = 0;
    ...
    if(map[max] > dat)
    {
        memmove(&map[max + 1],&map[max],(*num - max)*sizeof(u32));
        map[max] = dat;
    }
    else if(map[max] < dat)
    {
        memmove(&map[max + 2],&map[max + 1],(*num - max - 1)*sizeof(u32));
        max++;
        map[max] = dat;
...
{
    u16 index = 0;
    if(FindMap32(map,*num,dat,&index))
    {
        memmove(&map[index],&map[index + 1],(*num - index - 1)*sizeof(u32));
        *num = *num - 1;
        return true;
    }
    return false;
...
{  
    ...
    //u32 crc32;
	while (len--) crc = (crc >> 8) ^ CRC32_TABLE[(crc & 0xff) ^ *data++];
	crc ^= 0xFFFFFFFF;
	crc &= crcmask;
...
u32 CommonGetCrc32Init(u32 crc_init ,u8 *data,u32 len)  
{  
    u32 crc = crc_init;
    while (len--) crc = (crc >> 8) ^ CRC32_TABLE[(crc & 0xff) ^ *data++];
    return crc;
...
static const char AddrByteTable[] = 
{
...
void GetAddrString(const u8 addr[6],char strAddr[13])
{
	...
	for(;i < 6;i++)
	{
		strAddr[2*i] = AddrByteTable[addr[i]>>4];
		strAddr[2*i + 1] = AddrByteTable[addr[i]&0x0f];
	}
	strAddr[12] = '\0';
}
