...
typedef enum
{
	...
	TMR_OPT_CALL_PERIOD_IMM,	//周期性执行，但设定定时器后会立即先执行一次
}TMR_OPT;

typedef enum
{
    TMR_RESET_IMMEDIATELY,  //立即重设定时器
    TMR_RESET_NEXT,         //本次超时后才重设定时器
}TMR_RESET_OPT;
...
typedef struct
{
    List TmrList;			//定时器列表
    OS_MEM TmrMempool;		//
	TMR_FUN_CALL Tmrmem[MAX_TMR_NUM];	//定时器资源池
}TMR_CALL;
...
//重置一个定时器时间
bool TmrReset(TMR_CALL* tmrList,int,u32 ms,TMR_RESET_OPT);
