...
#define HPLC_COMMAND_TASK_PRIO   13
#define HPLC_COMMAND_TASK_STACK_SIZE        256

OS_TCB  Command_Bplc_TCB;
CPU_STK  Command_BplcStk[HPLC_COMMAND_TASK_STACK_SIZE];

//字节通知信号量
static OS_SEM CharSem;
...
*/
    OS_ERR err;
    OSSemPend (&CharSem,0,OS_OPT_PEND_BLOCKING,0,&err);
    if(err == OS_ERR_NONE)
    {
        return FinshGetChar();
    }
...
static void finsh_receive_info(void)
{
    OS_ERR err;
    OSSemPost(&CharSem,OS_OPT_POST_ALL,&err);
}
...
int finsh_system_init(void)
{
    ...
    */
    FinshDeviceInit(115200);
    FinshCharCallback(finsh_receive_info);
    OSSemCreate(&CharSem,"Char SEM",0,&os_err);
    OSTaskCreate(&Command_Bplc_TCB,
             "Command Thread",
              finsh_thread_entry,
              0,
              HPLC_COMMAND_TASK_PRIO,
              &Command_BplcStk[0],
              HPLC_COMMAND_TASK_STACK_SIZE / 10u,
              HPLC_COMMAND_TASK_STACK_SIZE,
              0u,
              0u,
              0,
    ...
    //rt_sem_init(&(shell->rx_sem), "shrx", 0, 0); 目前没使用
