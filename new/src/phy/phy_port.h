...
extern const int16_t rx_band2_bpf_gain_gw[6];
extern const int16_t rx_band2_bpf_a1_gw[6];
extern const int16_t rx_band2_bpf_a2_gw[6];
extern const int16_t rx_band2_bpf_gain_nw[6];
extern const int16_t rx_band2_bpf_a1_nw[6];
extern const int16_t rx_band2_bpf_a2_nw[6];
...
extern const int16_t rx_band2_comp_phase_gw[512];
extern const int16_t rx_band2_comp_phase_nw[512];
...
typedef struct
{
  ...
  //  uint32_t len;         //MPDU(帧控制+帧载荷)长度
	uint8_t link;       //接收帧类型 0 HPLC  1HRF
  union
  {
    struct
    {
...
#define LOW_NBI_GAIN                  11 //单相线接收  NBI GAIN
#define HIGH_NBI_GAIN                 15//全相线接收 NBI GAIN
#ifndef FPGA_ENV
