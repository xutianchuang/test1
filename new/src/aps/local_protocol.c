...
//97协议读取地址
const u8 ReadMeterAddr97[16] = 
{
    0xFE,0xFE,0x68,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0x68,0x01,0x02,0x43,0xC3,0xD5,0x16
};

//07协议读取地址
const u8 ReadMeterAddr07[14] = 
{
    0xFE,0xFE,0x68,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0x68,0x13,0x00,0xDF,0x16
};
...
u16 GetInitCrc16(u16 crcinit,const u8 * src,const u16 len)
{
	...
	u16 i;
	for(i = 0; i < len; i ++){
		crc = (crc << 8 ) ^ CRC16Table[ ((crc>>8) ^* src ++) & 0xFF];
	}
	return crc;
}
