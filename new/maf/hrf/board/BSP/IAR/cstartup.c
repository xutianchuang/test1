...
/* IAR编译器特定定义 */
    ...
    #define SECTION_VECTORS         _Pragma("location=\".vectors\"")
    #define KEEP_SYMBOL             __root
    #define WEAK_SYMBOL             __weak
    #define SECTION_BEGIN(name)     __section_begin(name)
...
WEAK_SYMBOL void OS_CPU_PendSVHandler(void)
{
    while(1);
}
WEAK_SYMBOL void OS_CPU_SysTickHandler(void)
{
    while(1);
}
 
WEAK_SYMBOL void USART0_IRQHandler(void)
{
	while(1);
}

WEAK_SYMBOL void USART1_IRQHandler(void)
{
	while(1);
}

WEAK_SYMBOL void USART2_IRQHandler(void)
{
	while(1);
}

WEAK_SYMBOL void USART3_IRQHandler(void)
{
	while(1);
}
WEAK_SYMBOL void HRF_dfe_interrupt_handler(void)
{
	while(1);
}
...
KEEP_SYMBOL  const  APP_INTVECT_ELEM  __vector_table[] = {
#elif defined ( __GNUC__ )
SECTION_VECTORS KEEP_SYMBOL const APP_INTVECT_ELEM __vector_table[] = {
#endif

    { .Ptr = STACK_TOP },                                   /*  0, SP start value.                                    */
#if defined ( __ICCARM__ )
    ...
    OS_TABLE_FUNC(DetectADC12VIrq),
    DummyHandlerROM, //43
    DummyHandlerROM, //44
    OS_TABLE_FUNC(HRF_dfe_interrupt_handler), //45
    (CPU_FNCT_VOID)FACTORY_CODE_FLAG,
};
...
int __low_level_init( void )
{
	...
	unsigned int *src = SECTION_BEGIN(".vectors");
...
WEAK_SYMBOL void WWDG_Handler(void)
{
    RESET_FLASH();
	while(1)
	{
