...
void FLASH_CMD_CONFIG(void)
{
	...
	QSPI->LUT_b[41].OPRND1 = 0x8;


	QSPI->LUT_b[42].INSTR0 = STOP;
	QSPI->LUT_b[42].PAD0 = 0x00;
	QSPI->LUT_b[42].OPRND0 = 0x00;	
	...
	QSPI->LUT_b[61].INSTR0 = WRITE;
	QSPI->LUT_b[61].PAD0 = 0x00;
	QSPI->LUT_b[61].OPRND0 = 0x8;
	QSPI->LUT_b[61].INSTR1 = STOP;
	QSPI->LUT_b[61].PAD1 = 0x00;
	QSPI->LUT_b[61].OPRND1 = 0;
...
{
	memcpy((u8 *)(pdata), (u8 *)((Package_start & 0xFFFFFF) | FLASH_BASE), wordsize * 4);
}
...
{ 
	...
	IPCR_SEQ(Write_Enable_SPI_novolatile_ID, 1);
	WAIT_REM32(QSPI->SR_b.BUSY, 0);
    QSPI->SFAR = 0x13000000;
	IPCR_SEQ(Erase_Securityreg_ID, 0x00);
	WAIT_REM32(QSPI->SR_b.BUSY, 0);
	Flash_Wait_BUSY();
...
void FLASH_ProgramSecurityBytes(uint32_t reg_num, uint32_t addr, uint8_t *pdata, uint32_t bytes)
{
	...
	while (1)
	{
		...
		if(currentdatalen != 0)//page in word
		{
			...
			//Flash_Wait_BUSY();
			pagebytes = currentdatalen * 4;
			IPCR_SEQ(Program_Securityreg_ID, pagebytes);
			WAIT_REM32(QSPI->SR_b.BUSY, 0);
			Flash_Wait_BUSY();
		}
		if (leavedatalen == 0)//less than one word
