...
typedef struct _USART_TRANSFER_INFO
{
	...
	void*                   dma_lli; 
	uint32_t                lli_idx;       // link mode idx done
} USART_TRANSFER_INFO;
...
//#define ARM_USART_CONTROL_TX_LLI           	(0x20UL << ARM_USART_CONTROL_Pos)   ///tx link mode
#define ARM_USART_CONTROL_RX_LLI           	(0x21UL << ARM_USART_CONTROL_Pos)   //rx link mode 


#define ARM_USART_EVENT_RECEIVE_HALF_COMPLETE       (1UL << 14)  
#define ARM_USART_EVENT_RECEIVE_NO_DATA_COMPLETE    (1UL << 15)
